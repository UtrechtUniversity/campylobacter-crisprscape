...
if  [ -f resources/PLSDB/download_meta.tar.gz ]; then
    ...
    wget -P resources/PLSDB https://ccb-microbe.cs.uni-saarland.de/plsdb2025/download_meta.tar.gz
fi

echo "Extracting download"
if [ -f resources/PLSDB/sequences.fasta.bz2 ]; then
    echo "already extracted, skipping..."
else
    tar -xzf resources/PLSDB/download_meta.tar.gz -C resources/PLSDB/
fi
echo "Unzipping sequences"
bzip2 -d resources/PLSDB/sequences.fasta.bz2
...
for DB in "Genbank" "RefSeq" "DDBJ" "EMBL" "PhagesDB" "GPD" "GVD" "MGV" "TemPhD" "CHVD" "IGVD" "IMG_VR" "GOV2" "STV"
    do
    if test -f "resources/phagescope/$DB.tar.gz"; then
        echo "Already downloaded $DB, skipping..."
    else
        echo "Downloading $DB"
        wget -O "resources/phagescope/$DB.tar.gz"  "https://phageapi.deepomics.org/download/phage/fasta/?datasource=$DB"
    fi
done

echo "Extracting databases"
parallel --jobs "$threads" 'DB={}; path=resources/phagescope; [ -f "$path/$DB.fasta" ] || [ -d "$path/$DB" ] && \
...
parallel --jobs "$threads" 'DB={}; path=resources/phagescope; [ -f "${path}/${DB}.fasta" ] && \
...
for DB in "genbank" "refseq" "ddbj" "embl" "phagesdb" "gpd" "gvd" "mgv" "temphd" "chvd" "igvd" "img_vr" "gov2" "stv"
    do
    if [ -f "resources/phagescope/${DB}_phage_meta_data.tsv" ]; then
        echo "Already downloaded $DB, skipping..."
    else
        echo "Downloading $DB"
        wget -P resources/phagescope/ "https://phageapi.deepomics.org/files/Download/Phage_meta_data/${DB}_phage_meta_data.tsv"
    fi
done

echo "Merging phagescope metadata"
first=true
if test -f "resources/phagescope/merged_metadata.tsv"; then
    ...
    for DB in "genbank" "refseq" "ddbj" "embl" "phagesdb" "gpd" "gvd" "mgv" "temphd" "chvd" "igvd" "img_vr" "gov2" "stv";
        ...
        echo "Merging $DB"
        if [ "$first" == true ]; then
            cat "resources/phagescope/${DB}_phage_meta_data.tsv" > resources/phagescope/merged_metadata.tsv
            first=false
        else
            tail -n +2 "resources/phagescope/${DB}_phage_meta_data.tsv" >> resources/phagescope/merged_metadata.tsv
        fi
    done 
fi
echo "Done!"
